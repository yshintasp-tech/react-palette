...
const palettes = {
  ...
  Vintage: [['Rosewood veil', ['#9a5f5d', '#ead9ce', '#4c3837', '#bc9d79']], ['Cloud latte', ['#d8c4ae', '#faf7f1', '#70817d', '#a47460']], ['Plum shadow', ['#573d57', '#c2a9bb', '#e4d7ca', '#2f3542']]],
}
const outfitOptions = ['Atasan', 'Bawahan', 'Jaket', 'Hijab', 'Sepatu']
const categories = Object.keys(palettes)

function App() {
  const [category, setCategory] = useState('Casual')
  const [selectedItems, setSelectedItems] = useState(['Atasan', 'Bawahan', 'Sepatu'])
  ...
  const savePalette = () => { setSavedPalettes((items) => [{ name: paletteName, colors, category, outfit: selectedItems, id: Date.now(), comments: [] }, ...items.filter((item) => item.name !== paletteName || item.category !== category)]); flash('Kombinasi disimpan ke library') }
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><a className="brand" href="#top"><span>CP</span> ChoosePalette</a><nav><a className="active" href="#studio">Studio</a><a href="#library">Library <b>{savedPalettes.length}</b></a></nav><div className="profile">NP <span>Nadia Putri</span></div></header>
